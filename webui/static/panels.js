...
//
// The assignee field auto-completes against the union of (a) live Iris
...
function switchSettingsSection(name,opts){
  ...
  let section=(name==='appearance'||name==='preferences'||name==='providers'||name==='plugins'||name==='extensions'||name==='knowledge'||name==='system'||name==='help')?name:'conversation';
...
async function _buildSettingsIndex() {
  ...
  const promise = (async () => {
    ...
    await Promise.all([loadProvidersPanel(), loadPluginsPanel(), loadExtensionsPanel(), loadKnowledgePanel()]);
    const index = [];
    const add = (entry) => {
    ...
    const sectionMap = {
      ...
      settingsPaneKnowledge: 'knowledge',
...
function _resolveSettingsField(entry) {
  ...
  const paneIds = {
    ...
    knowledge: 'settingsPaneKnowledge',
...
async function loadSettingsPanel(){
  try{
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'Iris';
...
async function loadKnowledgePanel(){
  const list=$('kbDocsList');
  const empty=$('kbDocsEmpty');
  if(!list||!empty) return;
  _initKnowledgePanelEvents();
  try{
...
async function _uploadKnowledgeFiles(files){
...
function _initKnowledgePanelEvents(){
  const up=$('kbUploadBtn'), file=$('kbFileInput');
  if(up&&file){
    up.onclick=()=>{ _uploadKnowledgeFiles(file.files); };
  }
  const sb=$('kbSearchBtn');
  if(sb) sb.onclick=_kbTestSearch;
  const si=$('kbSearchInput');
  if(si) si.addEventListener('keydown',e=>{ if(e.key==='Enter') _kbTestSearch(); });
}

async function loadExtensionsPanel(opts){
  const target=$('extensionsDiagnostics');  const copyBtn=$('extensionsCopyDiagnosticsBtn');
  if(!target) return;
...
// ── Plugins panel (Iris: install / remove / manage) ─────────────────────────
...
// ── Iris: 插件安装 / 卸载（复用 hermes CLI，经 /api/plugins/* 端点） ──────────

async function loadPluginCatalog(){
  const box=$('pluginCatalog');
  if(!box) return;
  try{
    const data=await api('/api/plugins/catalog');
    const raw=(data||{}).plugins||{};
    const plugins=Array.isArray(raw)?raw:(Array.isArray(raw.results)?raw.results:[]);
    if(!plugins.length){
      box.style.display='none';
      box.innerHTML='';
      return;
...
async function installPluginByIdentifier(identifier,btn){
...
async function removePluginByName(name,btn){
  ...
  setStatus('Removing '+name+' …',false);
  try{
    const data=await api('/api/plugins/remove',{method:'POST',body:JSON.stringify({name:name}),timeoutMs:120000});
    if(data&&data.ok){
      setStatus('Removed: '+name,false);
      loadPluginsPanel();
    }else{
...
function _bindPluginInstallBox(){
  const input=$('pluginInstallInput');
  const btn=$('pluginInstallBtn');
  if(!input||!btn) return;
  btn.addEventListener('click',function(){
    installPluginByIdentifier(input.value.trim(),btn);
  });
...
const enabled=plugin&&plugin.enabled!==false;
  ...
  if(!isDashboardPlugin&&plugin&&plugin.key){
    const _rm=card.querySelector('.plugin-remove-btn');
    if(_rm){
      const _k=plugin.key;
      _rm.addEventListener('click', function(){ removePluginByName(_k, this); });
    }
  }
  return card;
}
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'Iris';
  if(typeof applyBotName==='function') applyBotName();
