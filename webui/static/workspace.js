  ...
  for(let attempt=0;attempt<maxAttempts;attempt++){
    ...
    try{
      ...
      const requestPromise=(async()=>{
        const res=await fetch(url.href,{credentials:'include',headers:(typeof FormData!=='undefined'&&(fetchOpts.body instanceof FormData))?undefined:{'Content-Type':'application/json'},...fetchOpts});
        if(!res.ok){
...
// Canonical Iris mutators plus MCP filesystem aliases that can create/edit files.
